...
function AccueilPortailJoueur() {
  ...
  const events_before: EventBefore[] = [
    {
      ...
      Win: "OUI",
      KDA: "47/04/08",
      Commentaire: "Il a carry la team, c'est le goat...",
    },
    {
      Nom: "Entraînement Fortnite",
      date: "27/11/2025",
      presence: "non",
      Win: "NA",
      KDA: "NA",
  ...
  const events_after: EventAfter[] = [
    {
      Nom: "Entraînement LoL",
      date: "24/12/2025",
      Commentaire: "Macro-games. Celui qui sera en retard jouera Yuumi.",
    },
  ];

  const [activePastEvent, setActivePastEvent] = useState<number | null>(null);
  const [activeFutureEvent, setActiveFutureEvent] = useState<number | null>(
    null
  );
  ...
  const togglePastEvent = (index: number) => {
    ...
    if (pastEventRefs.current[index]) {
      pastEventRefs.current[index].scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
  ...
  const toggleFutureEvent = (index: number) => {
    ...
    if (futureEventRefs.current[index]) {
      futureEventRefs.current[index].scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
      ...
      <div className="body-portail-joueur">
        ...
        <div className="body-portail-joueur-right">
          <div className="body-portail-joueur-right-body">
              ...
              return (
                <div key={i} className="event-container">
                  <button
                    ...
                    onClick={() => togglePastEvent(i)}
                  >
                    {event.Nom}
                  </button>

                  <div
                    className={`event-passé ${activePastEvent === i ? "active" : ""}`}
                    ref={(el) => {
                      if (el) pastEventRefs.current[i] = el;
                    }}
                  >
                    <div className="présence-joueur">
                      <div
                        className={`bar-présence ${presenceValue === "oui" ? "green" : presenceValue === "non" ? "red" : "gray"}`}
                      ></div>
                      {presenceValue === "oui"
                        ? "Présent !"
                        : presenceValue === "non"
                      ...
                      {(Object.keys(event) as (keyof EventBefore)[])
                        ...
                        .map((key, j) => (
                          <div className="stat-joueur-line" key={j}>
                            {key}
                            <div className="chiffre-stats-joueur">
                              {event[key]}
                            </div>
                          </div>
                        ))}
            ...
            <div className="title-right-portail-joueur FUTUR">
            ...
            {events_after.map((event, i) => (
              <div key={i} className="event-container">
                <button
                  className="event-toggle-button"
                  onClick={() => toggleFutureEvent(i)}
                >
                  {event.Nom}
                </button>

                <div
                  className={`event-passé ${activeFutureEvent === i ? "active" : ""}`}
                  ref={(el) => {
                    if (el) futureEventRefs.current[i] = el;
                  }}
                >
                  <div className="stats-joueur">
                    {(Object.keys(event) as (keyof EventAfter)[]).map(
                      (key, j) => (
                        <div className="stat-joueur-line" key={j}>
                          {key}
                          <div className="chiffre-stats-joueur">
